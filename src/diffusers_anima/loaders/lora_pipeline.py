    ...
    *,
        ...
        if weight_name is not None:
            ...
            if not os.path.isfile(candidate):
                raise FileNotFoundError(
                    f"LoRA weight '{weight_name}' not found in '{base}'."
                )
        ...
        if weight_name is not None:
            ...
            try:
                model_file = hf_hub_download(
                    repo_id=path, filename=_LORA_WEIGHT_NAME_SAFE, **dl_kwargs
                )
            except EntryNotFoundError:
                if not allow_pickle:
                    raise
                model_file = hf_hub_download(
                    repo_id=path, filename=_LORA_WEIGHT_NAME, **dl_kwargs
                )
        else:
            model_file = hf_hub_download(
                repo_id=path, filename=_LORA_WEIGHT_NAME, **dl_kwargs
    ...
    if is_safetensors:
        ...
        logger.warning(
            ...
            "For security, prefer safetensors format.",
            model_file,
        )
        ...
        raise ValueError(
            f"'{model_file}' is not a safetensors file. "
            "Pass allow_pickle=True to load pickle files."
        )
...
def _alpha_scales_from_down_weight(
    *, down_weight: torch.Tensor, alpha_value: float
) -> tuple[float, float]:
...
def _write_down_up_lora_payload(
    *,
    ...
    if alpha is None:
        ...
        scale_down, scale_up = _alpha_scales_from_down_weight(
            down_weight=down_weight, alpha_value=float(alpha)
        )
    ...
    if down_bias is not None:
        if not torch.is_tensor(down_bias):
            raise ValueError(
                f"Invalid LoRA tensor payload for module '{module_path}.lora_down.bias'."
            )
        converted[f"transformer.{module_path}.lora_A.bias"] = down_bias
    if up_bias is not None:
        if not torch.is_tensor(up_bias):
            raise ValueError(
                f"Invalid LoRA tensor payload for module '{module_path}.lora_up.bias'."
            )
...
def _convert_non_diffusers_anima_lora_to_diffusers(
    ...
    if len(converted) == 0:
        raise ValueError(
            "No loadable LoRA weights were found in the provided state dict."
        )
    return converted
...
class AnimaLoraLoaderMixin(LoraBaseMixin):
    ...
    def lora_state_dict(
        ...
        **kwargs,
        ...
        converted_state_dict = _convert_non_diffusers_anima_lora_to_diffusers(
            state_dict
        )
    ...
    def save_lora_weights(
        ...
        transformer_lora_layers: dict[str, torch.nn.Module | torch.Tensor]
        | None = None,
...
def _resolve_pipeline_transformer(pipeline: object, transformer_name: str):
    ...
    if transformer is None:
        raise ValueError(
            f"Could not resolve transformer component '{transformer_name}' from pipeline."
        )
    return transformer
