...
def randn_tensor(
    ...
    *,
    ...
    if generator_device == target_device.type:
        return torch.randn(
            shape, device=target_device, dtype=dtype, generator=generator
        )

    noise = torch.randn(
        shape, device=generator.device, dtype=torch.float32, generator=generator
    )
...
def sample_flowmatch_euler(
    ...
    *,
    ...
    if sigma_schedule != "uniform":
        raise ValueError(
            "flowmatch_euler sampler only supports sigma_schedule='uniform'."
        )
    if inpaint_mask is not None and (init_image_latents is None or init_noise is None):
        raise ValueError(
            "inpaint sampling requires both `init_image_latents` and `init_noise`."
    ...
    for i, timestep in enumerate(_iterable):
        ...
        if (
            ...
            if i < len(timesteps) - 1:
                ...
                source_latents = scheduler.scale_noise(
                    init_image_latents, next_timestep, init_noise
                )
            latents = (1.0 - inpaint_mask) * source_latents + inpaint_mask * latents
        latents = _run_step_callback(
            pipeline,
