...
def _reshape_image_tensor_to_bchw(
    ...
    *,
    ...
    if tensor.ndim == 2:
        ...
        raise ValueError(
            f"`{input_label}` must be 2D/3D/4D. Got shape {tuple(tensor.shape)}."
        )

    if tensor.shape[0] < 1:
        raise ValueError(
            f"`{input_label}` batch size must be >= 1. Got {tensor.shape[0]}."
        )
    return tensor
...
def prepare_init_image_tensor(
    ...
    *,
    ...
    if isinstance(image, (list, tuple)):
        ...
        pil_image = image.convert("RGB").resize(
            (width, height), Image.Resampling.LANCZOS
        )
        tensor = (
            torch.from_numpy(np.array(pil_image, copy=True))
            .permute(2, 0, 1)
            .unsqueeze(0)
        )
    else:
        ...
        if tensor.shape[1] == 1:
            ...
            raise ValueError(
                f"`image` must have 1 or 3 channels, got {tensor.shape[1]}."
            )
...
def prepare_inpaint_mask_tensor(
    ...
    *,
    ...
    if isinstance(mask_image, (list, tuple)):
        ...
        pil_mask = mask_image.convert("L").resize(
            (width, height), Image.Resampling.LANCZOS
        )
        ...
        if mask.shape[1] == 3:
            ...
            raise ValueError(
                f"`mask_image` must have 1 or 3 channels, got {mask.shape[1]}."
            )
...
def _ensure_finite(
    tensor: torch.Tensor, *, name: str, runtime_dtype: torch.dtype
) -> None:
