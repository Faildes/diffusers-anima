...
def _convert_anima_vae_mid_resnet_key(
    key: str, *, prefix: str, source_index: int, target_index: int
) -> str | None:
...
def _convert_anima_vae_decoder_upsample_key(key: str) -> str | None:
    ...
    if idx in _ANIMA_VAE_DECODER_UP_UPSAMPLER_MAP and tail.startswith(
        ("resample.", "time_conv.")
    ):
