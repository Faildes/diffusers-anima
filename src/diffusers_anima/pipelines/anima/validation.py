...
_ANIMA_SINGLE_FILE_FROM_PRETRAINED_KEYS = (
    _ANIMA_RUNTIME_OPTION_KEYS
    | _ANIMA_LOADER_OPTION_KEYS
    | _ANIMA_COMPONENT_INSTANCE_KEYS
)
...
def _validate_image_like_input(
    image: ImageBatchInput | None, *, input_name: str
) -> None:
    if image is None:
        return
    ...
    *,
    ...
    if ignored:
        warnings.warn(
            ...
            f"sampler='{sampler}', sigma_schedule='{sigma_schedule}': "
            + ", ".join(sorted(ignored)),
            stacklevel=3,
        )
...
def _raise_if_removed_from_pretrained_runtime_feature_kwargs(
    ...
    *,
    ...
    removed = sorted(
        key
        for key in _ANIMA_REMOVED_FROM_PRETRAINED_RUNTIME_FEATURE_KEYS
        if key in kwargs
    )
    if len(removed) == 0:
        return
...
def _partition_single_file_from_pretrained_kwargs(
    ...
    ignored = sorted(
        key
        for key in kwargs
        if key in _DIFFUSERS_COMPAT_IGNORED_SINGLE_FILE_FROM_PRETRAINED_KEYS
    )
    unknown = sorted(
        key
        for key in kwargs
        if key not in _ANIMA_SINGLE_FILE_FROM_PRETRAINED_KEYS and key not in ignored
    )
